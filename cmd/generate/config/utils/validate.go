...
func ValidateWithPaths(rule config.Rule, truePositives map[string]string, falsePositives map[string]string) *config.Rule {
	...
	for path, tp := range truePositives {
		f := sources.Fragment{Raw: tp, Attributes: []sources.Attribute{{Key: sources.AttrPath, Value: path}}}
		if len(d.Detect(f)) != 1 {
			logging.Fatal().
				Str("rule", r.RuleID).
				Str("value", tp).
	...
	for path, fp := range falsePositives {
		f := sources.Fragment{Raw: fp, Attributes: []sources.Attribute{{Key: sources.AttrPath, Value: path}}}
		if len(d.Detect(f)) != 0 {
			logging.Fatal().
				Str("rule", r.RuleID).
				Str("value", fp).
