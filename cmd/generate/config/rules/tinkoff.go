...
func TinkoffAPIToken() *config.Rule {
	r := config.Rule{
		...
		Regex:       utils.GenerateSemiGenericRegex([]string{"tinkoff", "tbank"}, `[a-z0-9]{50,100}`, true),
		Keywords:    []string{"tinkoff", "tbank"},
		Entropy:     3.5,
	}

	tps := utils.GenerateSampleSecrets("tinkoff", secrets.NewSecretWithEntropy(`[a-z0-9]{50,100}`, 3.5))
	fps := []string{
