...
func EtsyAccessToken() *config.Rule {
	...
	// validate
	tps := utils.GenerateSampleSecrets("ETSY", secrets.NewSecretWithEntropy(utils.AlphaNumeric("24"), 3))
	tps = append(tps, utils.GenerateSampleSecrets("etsy", secrets.NewSecretWithEntropy(utils.AlphaNumeric("24"), 3))...)
	tps = append(tps, utils.GenerateSampleSecrets("Etsy", secrets.NewSecretWithEntropy(utils.AlphaNumeric("24"), 3))...)
	fps := []string{
		fmt.Sprintf(`SetSysctl = "%s"`, secrets.NewSecretWithEntropy(utils.AlphaNumeric("24"), 3)),
