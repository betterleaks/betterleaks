...
func FlyIOAccessToken() *config.Rule {
	...
	// validate
	// fo1_
	tps := utils.GenerateSampleSecrets("fly", secrets.NewSecretWithEntropy(`fo1_[\w-]{43}`, 4))
	tps = append(tps,
...
fly deploy`)
	// fm1
	tps = append(tps, utils.GenerateSampleSecrets("fly", secrets.NewSecretWithEntropy(`fm1[ar]_[a-zA-Z0-9+\/]{100,}={0,3}`, 4))...)
	tps = append(tps,
	...
	// fm2
	tps = append(tps, utils.GenerateSampleSecrets("fly", secrets.NewSecretWithEntropy(`fm2_[a-zA-Z0-9+\/]{100,}={0,3}`, 4))...)
	tps = append(tps,
