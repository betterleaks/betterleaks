...
func SalesforceInstanceURL() *config.Rule {
	...
	r := config.Rule{
		...
		Regex: regexp.MustCompile(
			`(?i)\b((?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?){0,4}\.my\.salesforce\.com|[a-z]{2,8}[0-9]{1,4}\.salesforce\.com))\b`,
		),
...
func SalesforceAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		ValidateExpr: `let r = http.get("https://" + captures["salesforce-instance-url.1"] + "/services/data/v60.0/limits", {
