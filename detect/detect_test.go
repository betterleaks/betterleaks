...
func TestValidateCredentialPipeline(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			po := ProviderOptions{Statuses: []report.ValidationStatus{report.ValidationStatusValid}}
			option := WithValidation(po)
			if tc.enabled {
				option = WithAnalysis(po)
			}
			d := mustNewDetector(t, cfg, option, WithMinimumConfidence(ConfidenceHigh),
				WithExcludedPaths("betterleaks://validate"), WithIgnoredFingerprints(fingerprint.Sum([]byte("raw-secret"))))
...
: {"reason": "credential inputs changed between stages"}`,
	...
	for _, set := range findings[0].ComponentSets {
		...
		direct, err := d.ValidateCredential(t.Context(), Credential{
...
func TestValidateCredentialInputs(t *testing.T) {
	...
	d := mustNewDetector(t, cfg, WithAnalysis(ProviderOptions{}))
	for _, tc := range []struct {
		name  string
		input Credential
		want  string
	...
	_, err := mustNewDetector(t, cfg).ValidateCredential(t.Context(), Credential{RuleID: "primary", Secret: "key"})
	require.ErrorContains(t, err, "requires WithValidation or WithAnalysis")
	for _, expression := range []struct{ validation, analysis string }{{`???`, ``}, {`{"result":"valid"}`, `???`}} {
		bad := mustNewDetector(t, &config.Config{Rules: []config.Rule{{ID: "bad", Regex: `bad`, ValidateExpr: expression.validation, AnalyzeExpr: expression.analysis}}}, WithAnalysis(ProviderOptions{}))
...
func TestValidateCredentialProviderLimitsAndCancellation(t *testing.T) {
	...
	d := mustNewDetector(t, &config.Config{Rules: []config.Rule{{ID: "key", Regex: `key`, ValidateExpr: expression}}}, WithAnalysis(ProviderOptions{MaxRequestsPerTarget: 1}))
	for range 2 {
	...
	defer blocked.Close()
	d = mustNewDetector(t, &config.Config{Rules: []config.Rule{{ID: "blocked", Regex: `x`, ValidateExpr: fmt.Sprintf(`let r = http.get(%q, {}); {"result":"valid"}`, blocked.URL)}}}, WithValidation(ProviderOptions{}))
	ctx, cancel = context.WithCancel(t.Context())
	defer cancel()
...
func TestValidateCredentialMatchesScanAndSupportsConcurrentCalls(t *testing.T) {
	...
	_, err = d.Scan(t.Context(), &sources.Reader{Content: strings.NewReader(input.Secret), Attributes: input.Attributes}, func(f report.Finding) error {
	...
	_, err = d.Scan(t.Context(), &sources.Reader{Content: strings.NewReader(input.Secret)}, func(report.Finding) error { return nil })
	require.NoError(t, err)
	for range 8 {
...
{"result": access == "permitted" && response.status == 200 ? "valid" : "invalid"}`, server.URL),
	...
	denied := mustNewDetector(t, cfg, WithAnalysis(ProviderOptions{Debug: true}))
	...
	allowed := mustNewDetector(t, cfg, WithAnalysis(ProviderOptions{Debug: true, EnvVars: []string{"BETTERLEAKS_DIRECT_TEST"}}))
