...
// --------------------------------------------------------------------------
// New betterleaks printing.
//
// PrintFinding prints a finding to stdout including all metadata key-value
// pairs. This is the default format; use LegacyPrintFinding for the
// gitleaks-compatible fixed-field layout (--legacy).
// --------------------------------------------------------------------------

// PrintFinding prints a finding to stdout with optional color formatting,
// including every metadata key-value pair.
func PrintFinding(f betterleaks.Finding, noColor bool) {
	...
	// Print all metadata key-value pairs in sorted order.
	if len(f.Metadata) > 0 {
