...
func YandexAWSAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex: utils2.GenerateSemiGenericRegex([]string{"yandex"},
	...
	// validate
	tps := utils2.GenerateSampleSecrets("yandex", secrets.NewSecret(`YC[a-zA-Z0-9_\-]{38}`))
...
func YandexAPIKey() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("yandex", secrets.NewSecret(`AQVN[A-Za-z0-9_\-]{35,38}`))
	return utils2.Validate(r, tps, nil)
}
...
func YandexAccessToken() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("yandex", secrets.NewSecret(`t1\.[A-Z0-9a-z_-]+[=]{0,2}\.[A-Z0-9a-z_-]{86}[=]{0,2}`))
	return utils2.Validate(r, tps, nil)
}
