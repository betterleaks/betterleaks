...
func DigitalOceanPAT() *config.Rule {
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`dop_v1_[a-f0-9]{64}`, false),
		Entropy:     3,
		Keywords:    []string{"dop_v1_"},
	}

	tps := utils2.GenerateSampleSecrets("do", "dop_v1_"+secrets.NewSecret(utils2.Hex("64")))
...
func DigitalOceanOAuthToken() *config.Rule {
	r := config.Rule{
		...
		Entropy:     3,
		Regex:       utils2.GenerateUniqueTokenRegex(`doo_v1_[a-f0-9]{64}`, false),
		Keywords:    []string{"doo_v1_"},
	}

	tps := utils2.GenerateSampleSecrets("do", "doo_v1_"+secrets.NewSecret(utils2.Hex("64")))
	return utils2.Validate(r, tps, nil)
}
...
func DigitalOceanRefreshToken() *config.Rule {
	r := config.Rule{
		...
		Regex:    utils2.GenerateUniqueTokenRegex(`dor_v1_[a-f0-9]{64}`, true),
		Keywords: []string{"dor_v1_"},
	}

	tps := utils2.GenerateSampleSecrets("do", "dor_v1_"+secrets.NewSecret(utils2.Hex("64")))
	return utils2.Validate(r, tps, nil)
}
