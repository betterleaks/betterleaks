...
func SlackBotToken() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("bot", "xoxb-781236542736-2364535789652-GkwFDQoHqzXDVsC6GzqYUypD")
	tps = append(tps,
		...
		`"bot_token4": `+fmt.Sprintf(`"xoxb-%s-%s-%s"`, secrets.NewSecret(utils2.Numeric("13")), secrets.NewSecret(utils2.Numeric("12")), secrets.NewSecret(utils2.AlphaNumeric("24"))),
	)
	fps := []string{
...
func SlackUserToken() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("user", "xoxp-41684372915-1320496754-45609968301-e708ba56e1517a99f6b5fb07349476ef")
	tps = append(tps,
		...
		`"user_token6": `+fmt.Sprintf(`"xoxp-%s-%s-%s-%s"`, secrets.NewSecret(utils2.Numeric("12")), secrets.NewSecret(utils2.Numeric("13")), secrets.NewSecret(utils2.Numeric("13")), secrets.NewSecret(utils2.AlphaNumeric("32"))),
...
func SlackAppLevelToken() *config.Rule {
	...
	tps := utils2.GenerateSampleSecrets("slack", "xapp-1-A052FGTS2DL-5171572773297-610b6a11f4b7eb819e87b767d80e6575a3634791acb9a9ead051da879eb5b55e")
	tps = append(tps,
		...
		`"token4": `+fmt.Sprintf(`"xapp-1-A%s-%s-%s"`, secrets.NewSecret(utils2.Numeric("10")), secrets.NewSecret(utils2.Numeric("13")), secrets.NewSecret(utils2.AlphaNumeric("64"))),
	)
...
func SlackConfigurationToken() *config.Rule {
	...
	tps := utils2.GenerateSampleSecrets("access", "xoxe.xoxp-1-Mi0yLTM0MTQwNDE0MDE3Ni0zNjU5NDY0Njg4MTctNTE4MjA3NTQ5NjA4MC01NDEyOTYyODY5NzUxLThhMTBjZmI1ZWIzMGIwNTg0ZDdmMDI5Y2UxNzVlZWVhYzU2ZWQyZTZiODNjNDZiMGUxMzRlNmNjNDEwYmQxMjQ")
	tps = append(tps,
		...
		`"access_token3": "xoxe.xoxp-1-`+secrets.NewSecret(utils2.AlphaNumeric("163"))+`"`,
		...
		`"access_token5": "xoxe.xoxb-1-`+secrets.NewSecret(utils2.AlphaNumeric("165"))+`"`,
	)
	fps := []string{
...
func SlackConfigurationRefreshToken() *config.Rule {
	...
	tps := utils2.GenerateSampleSecrets("refresh", "xoxe-1-My0xLTMxNzcwMjQ0MTcxMy01MTU4MTUyNjkxNzE0LTUxODE4NDI0MDY3MzYtMjA5MGFkOTFlZThkZWE2OGFlZDYwYWJjODNhYzAxYjA5ZjVmODBhYjgzN2QyNDdjOTNlOGY5NTg2YWM1OGM4Mg")
	tps = append(tps,
		...
		`"refresh_token3": "xoxe-1-`+secrets.NewSecret(utils2.AlphaNumeric("146"))+`"`,
	)
...
func SlackLegacyBotToken() *config.Rule {
	...
	tps := utils2.GenerateSampleSecrets("slack", "xoxb-263594206564-FGqddMF8t08v8N7Oq4i57vs1")
	tps = append(tps,
		...
		`"bot_token10": `+fmt.Sprintf(`"xoxb-%s-%s`, secrets.NewSecret(utils2.Numeric("10")), secrets.NewSecret(utils2.AlphaNumeric("24"))), // gitleaks:allow
		`"bot_token11": `+fmt.Sprintf(`"xoxb-%s-%s`, secrets.NewSecret(utils2.Numeric("12")), secrets.NewSecret(utils2.AlphaNumeric("23"))), // gitleaks:allow
	)
	fps := []string{
...
func SlackLegacyWorkspaceToken() *config.Rule {
	...
	tps := utils2.GenerateSampleSecrets("slack", "xoxa-2-511111111-31111111111-3111111111111-e039d02840a0b9379c")
	tps = append(tps,
		`"access_token": "xoxa-2-511111111-31111111111-3111111111111-e039d02840a0b9379c"`, // gitleaks:allow
		`"access_token1": `+fmt.Sprintf(`"xoxa-%s-%s`, secrets.NewSecret(utils2.Numeric("1")), secrets.NewSecret(utils2.AlphaNumeric("12"))),
		`"access_token2": `+fmt.Sprintf(`"xoxa-%s`, secrets.NewSecret(utils2.AlphaNumeric("12"))),
		`"refresh_token1": `+fmt.Sprintf(`"xoxr-%s-%s`, secrets.NewSecret(utils2.Numeric("1")), secrets.NewSecret(utils2.AlphaNumeric("12"))),
		`"refresh_token2": `+fmt.Sprintf(`"xoxr-%s`, secrets.NewSecret(utils2.AlphaNumeric("12"))),
	)
	fps := []string{
		// "xoxa-faketoken",
...
func SlackLegacyToken() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("slack", "xoxs-416843729158-132049654-5609968301-e708ba56e1")
	tps = append(tps,
		...
		`"access_token6": "xoxs-`+fmt.Sprintf("%s-%s-%s-%s", secrets.NewSecret(utils2.Numeric("10")), secrets.NewSecret(utils2.Numeric("10")), secrets.NewSecret(utils2.Numeric("10")), secrets.NewSecret(utils2.Hex("10")))+`"`,
...
func SlackWebHookUrl() *config.Rule {
	...
	// validate
	tps := []string{
		"hooks.slack.com/services/" + secrets.NewSecret(utils2.AlphaNumeric("44")),
		"http://hooks.slack.com/services/" + secrets.NewSecret(utils2.AlphaNumeric("45")),
		"https://hooks.slack.com/services/" + secrets.NewSecret(utils2.AlphaNumeric("46")),
		...
		"hooks.slack.com/workflows/" + secrets.NewSecret(utils2.AlphaNumeric("44")),
		"http://hooks.slack.com/workflows/" + secrets.NewSecret(utils2.AlphaNumeric("45")),
		"https://hooks.slack.com/workflows/" + secrets.NewSecret(utils2.AlphaNumeric("46")),
		...
		"https://hooks.slack.com/triggers/" + secrets.NewSecret(utils2.AlphaNumeric("56")),
	}
	return utils2.Validate(r, tps, nil)
}
