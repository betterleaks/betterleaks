...
func SentryAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"sentry"}, utils2.Hex("64"), true),
		Entropy:     3,
		Keywords: []string{
			"sentry",
		},
	...
	// validate
	tps := utils2.GenerateSampleSecrets("sentry", secrets.NewSecret(utils2.Hex("64")))
...
func SentryOrgToken() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("sentry",
	...
	tps = append(tps, utils2.GenerateSampleSecrets("sentry",
...
func SentryUserToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`sntryu_[a-f0-9]{64}`, false),
		Entropy:     3.5,
		Keywords:    []string{"sntryu_"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("sentry", secrets.NewSecret(`sntryu_[a-f0-9]{64}`))
	fps := []string{
