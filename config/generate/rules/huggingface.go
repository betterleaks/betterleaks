...
func HuggingFaceAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex("hf_(?i:[a-z]{34})", false),
		Entropy:     2,
		Keywords: []string{
			"hf_",
		},
	...
	// validate
	tps := utils2.GenerateSampleSecrets("huggingface", "hf_"+secrets.NewSecret("[a-zA-Z]{34}"))
	tps = append(tps,
...
func HuggingFaceOrganizationApiToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex("api_org_(?i:[a-z]{34})", false),
	...
	// validate
	tps := utils2.GenerateSampleSecrets("huggingface", "api_org_"+secrets.NewSecret("[a-zA-Z]{34}"))
	tps = append(tps,
