...
func VaultServiceToken() *config2.Rule {
	// define rule
	r := config2.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`(?:hvs\.[\w-]{90,120}|s\.(?i:[a-z0-9]{24}))`, false),
		Entropy:     3.5,
	...
	tps := []string{
		// Old
		utils2.GenerateSampleSecret("vault", secrets.NewSecret(`s\.[0-9][a-zA-Z0-9]{23}`)),
		`token: s.ZC9Ecf4M5g9o34Q6RkzGsj0z`,
		// New
		utils2.GenerateSampleSecret("vault", secrets.NewSecret(`hvs\.[0-9][\w\-]{89}`)),
...
func VaultBatchToken() *config2.Rule {
	// define rule
	r := config2.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`hvb\.[\w-]{138,300}`, false),
		Entropy:     4,
		Keywords:    []string{"hvb."},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("vault", "hvb."+secrets.NewSecret(utils2.AlphaNumericExtendedShort("138")))
