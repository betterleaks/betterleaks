...
func FacebookSecret() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"facebook"}, utils2.Hex("32"), true),
		Entropy:     3,
		Keywords:    []string{"facebook"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("facebook", secrets.NewSecret(utils2.Hex("32")))
	tps = append(tps,
...
func FacebookAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`\d{15,16}(\||%)[0-9a-z\-_]{27,40}`, true),
		Keywords:    []string{"facebook"},
		Entropy:     3,
	}
...
func FacebookPageAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex("EAA[MC](?i)[a-z0-9]{100,}", false),
		Entropy:     4,
