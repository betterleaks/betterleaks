...
func CloudflareGlobalAPIKey() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex(identifiers, utils2.Hex("37"), true),
		Entropy:     2,
		Keywords:    identifiers,
	}

	// validate
	tps := utils2.GenerateSampleSecrets("cloudflare", secrets.NewSecret(utils2.Hex("37")))
...
func CloudflareAPIKey() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex(identifiers, utils2.AlphaNumericExtendedShort("40"), true),
		Entropy:     2,
		Keywords:    identifiers,
	}

	// validate
	tps := utils2.GenerateSampleSecrets("cloudflare", secrets.NewSecret(utils2.AlphaNumericExtendedShort("40")))
...
func CloudflareOriginCAKey() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`v1\.0-`+utils2.Hex("24")+"-"+utils2.Hex("146"), false),
		Entropy:     2,
		Keywords:    ca_identifiers,
	}

	// validate
	tps := utils2.GenerateSampleSecrets("cloudflare", "v1.0-aaa334dc886f30631ba0a610-0d98ef66290d7e50aac7c27b5986c99e6f3f1084c881d8ac0eae5de1d1aa0644076ff57022069b3237d19afe60ad045f207ef2b16387ee37b749441b2ae2e9ebe5b4606e846475d4a5")
