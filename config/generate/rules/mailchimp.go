...
func MailChimp() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"MailchimpSDK.initialize", "mailchimp"}, utils2.Hex("32")+`-us\d\d`, true),
	...
	// validate
	tps := utils2.GenerateSampleSecrets("mailchimp", secrets.NewSecret(utils2.Hex("32"))+"-us20")
	tps = append(tps,
