...
var githubAllowlist = []*config2.Allowlist{
	{
		Paths: []*regexp.Regexp{
...
func GitHubPat() *config2.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("github", "ghp_"+secrets.NewSecret(utils2.AlphaNumeric("36")))
	fps := []string{
...
func GitHubFineGrainedPat() *config2.Rule {
	// define rule
	r := config2.Rule{
	...
	// validate
	tps := utils2.GenerateSampleSecrets("github", "github_pat_"+secrets.NewSecret(utils2.AlphaNumeric("82")))
	fps := []string{
...
func GitHubOauth() *config2.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("github", "gho_"+secrets.NewSecret(utils2.AlphaNumeric("36")))
	fps := []string{
...
func GitHubApp() *config2.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("github", "ghs_"+secrets.NewSecret(utils2.AlphaNumeric("36")))
	tps = append(tps, utils2.GenerateSampleSecrets("github", "ghu_"+secrets.NewSecret(utils2.AlphaNumeric("36")))...)
	fps := []string{
...
func GitHubRefresh() *config2.Rule {
	// define rule
	r := config2.Rule{
	...
	// validate
	tps := utils2.GenerateSampleSecrets("github", "ghr_"+secrets.NewSecret(utils2.AlphaNumeric("36")))
	fps := []string{
