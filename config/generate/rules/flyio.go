...
func FlyIOAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`(?:fo1_[\w-]{43}|fm1[ar]_[a-zA-Z0-9+\/]{100,}={0,3}|fm2_[a-zA-Z0-9+\/]{100,}={0,3})`, false),
		Entropy:     4,
	...
	// validate
	// fo1_
	tps := utils2.GenerateSampleSecrets("fly", secrets.NewSecret(`fo1_[\w-]{43}`))
	tps = append(tps,
...
fly deploy`)
	// fm1
	tps = append(tps, utils2.GenerateSampleSecrets("fly", secrets.NewSecret(`fm1[ar]_[a-zA-Z0-9+\/]{100,}={0,3}`))...)
	tps = append(tps,
	...
	// fm2
	tps = append(tps, utils2.GenerateSampleSecrets("fly", secrets.NewSecret(`fm2_[a-zA-Z0-9+\/]{100,}={0,3}`))...)
	tps = append(tps,
