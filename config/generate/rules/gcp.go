...
func GCPServiceAccount() *config2.Rule {
	// define rule
	r := config2.Rule{
...
func GCPAPIKey() *config2.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("gcp", secrets.NewSecret(`AIza[\w-]{35}`))
	tps = append(tps,
