...
func TwitterAPIKey() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"twitter"}, utils2.AlphaNumeric("25"), true),
		Keywords:    []string{"twitter"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("twitter", secrets.NewSecret(utils2.AlphaNumeric("25")))
...
func TwitterAPISecret() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"twitter"}, utils2.AlphaNumeric("50"), true),
		Keywords:    []string{"twitter"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("twitter", secrets.NewSecret(utils2.AlphaNumeric("50")))
	return utils2.Validate(r, tps, nil)
}
...
func TwitterBearerToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"twitter"}, "A{22}[a-zA-Z0-9%]{80,100}", true),

		Keywords: []string{"twitter"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("twitter", secrets.NewSecret("A{22}[a-zA-Z0-9%]{80,100}"))
	return utils2.Validate(r, tps, nil)
}
...
func TwitterAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"twitter"}, "[0-9]{15,25}-[a-zA-Z0-9]{20,40}", true),
		Keywords:    []string{"twitter"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("twitter", secrets.NewSecret("[0-9]{15,25}-[a-zA-Z0-9]{20,40}"))
	return utils2.Validate(r, tps, nil)
}
...
func TwitterAccessSecret() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"twitter"}, utils2.AlphaNumeric("45"), true),
		Keywords:    []string{"twitter"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("twitter", secrets.NewSecret(utils2.AlphaNumeric("45")))
	return utils2.Validate(r, tps, nil)
}
