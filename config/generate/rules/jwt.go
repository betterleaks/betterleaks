...
func JWT() *config.Rule {
	...
	r := config.Rule{
		...
		RuleID:      "jwt",
		Regex:       utils2.GenerateUniqueTokenRegex(`ey[a-zA-Z0-9]{17,}\.ey[a-zA-Z0-9\/\\_-]{17,}\.(?:[a-zA-Z0-9\/\\_-]{10,}={0,2})?`, false),
		Entropy:     3,
		Keywords:    []string{"ey"},
	}

	// validate
	tps := utils2.GenerateSampleSecrets("jwt", secrets.NewSecret(`ey[a-zA-Z0-9]{17,}\.ey[a-zA-Z0-9\/\\_-]{17,}\.(?:[a-zA-Z0-9\/\\_-]{10,}={0,2})?`))
	tps = append(tps,
