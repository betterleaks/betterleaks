...
func OnePasswordServiceAccountToken() *config.Rule {
	...
	// validate
	tps := []string{
		utils2.GenerateSampleSecret("1password", secrets.NewSecret(`ops_eyJ[a-zA-Z0-9+/]{250,}={0,3}`)),
...
func OnePasswordSecretKey() *config.Rule {
	...
	// validate
	tps := utils2.GenerateSampleSecrets("1password", secrets.NewSecret(`A3-[A-Z0-9]{6}-[A-Z0-9]{11}-[A-Z0-9]{5}-[A-Z0-9]{5}-[A-Z0-9]{5}`))
	tps = append(tps, utils2.GenerateSampleSecrets("1password", secrets.NewSecret(`A3-[A-Z0-9]{6}-[A-Z0-9]{6}-[A-Z0-9]{5}-[A-Z0-9]{5}-[A-Z0-9]{5}-[A-Z0-9]{5}`))...)
	tps = append(tps,
		// from whitepaper
