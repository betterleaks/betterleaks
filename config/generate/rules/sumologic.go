...
func SumoLogicAccessID() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:   utils2.GenerateSemiGenericRegex([]string{"(?-i:[Ss]umo|SUMO)"}, "su[a-zA-Z0-9]{12}", false),
		Entropy: 3,
		Keywords: []string{
			"sumo",
		},
	...
	// validate
	tps := utils2.GenerateSampleSecrets("sumo", secrets.NewSecret(`su[a-zA-Z0-9]{12}`))
	tps = append(tps,
...
func SumoLogicAccessToken() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateSemiGenericRegex([]string{"(?-i:[Ss]umo|SUMO)"}, utils2.AlphaNumeric("64"), true),
		Entropy:     3,
		Keywords: []string{
			"sumo",
		},
	...
	// validate
	tps := utils2.GenerateSampleSecrets("sumo", secrets.NewSecret(utils2.AlphaNumeric("64")))
	tps = append(tps,
