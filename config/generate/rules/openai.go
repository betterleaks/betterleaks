...
func OpenAI() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       utils2.GenerateUniqueTokenRegex(`sk-(?:proj|svcacct|admin)-(?:[A-Za-z0-9_-]{74}|[A-Za-z0-9_-]{58})T3BlbkFJ(?:[A-Za-z0-9_-]{74}|[A-Za-z0-9_-]{58})\b|sk-[a-zA-Z0-9]{20}T3BlbkFJ[a-zA-Z0-9]{20}`, false),
		Entropy:     3,
		Keywords: []string{
			"T3BlbkFJ",
		},
	...
	// validate
	tps := append(utils2.GenerateSampleSecrets("openaiApiKey", "sk-"+secrets.NewSecret(utils2.AlphaNumeric("20"))+"T3BlbkFJ"+secrets.NewSecret(utils2.AlphaNumeric("20"))),
		[]string{
