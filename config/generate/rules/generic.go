...
func GenericCredential() *config2.Rule {
	// define rule
	r := config2.Rule{
		...
		Allowlists: []*config2.Allowlist{
			...
			},
			{
				...
				MatchCondition: config2.AllowlistMatchOr,
				RegexTarget:    "match",
				Regexes: []*regexp.Regexp{
			...
			{
				MatchCondition: config2.AllowlistMatchAnd,
				RegexTarget:    "line",
				Regexes: []*regexp.Regexp{
	...
	// validate
	tps := utils2.GenerateSampleSecrets("generic", "CLOJARS_34bf0e88955ff5a1c328d6a7491acc4f48e865a7b8dd4d70a70749037443") //gitleaks:allow
	tps = append(tps, utils2.GenerateSampleSecrets("generic", "Zf3D0LXCM3EIMbgJpUNnkRtOfOueHznB")...)
	tps = append(tps,
		// Access
...
func newPlausibleSecret(regex string) string {
	allowList := &config2.Allowlist{StopWords: DefaultStopWords}
