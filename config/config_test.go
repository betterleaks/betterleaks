			...
			cfg: Config{
				Rules: map[string]Rule{
					"github-app-token": {
						...
						Prefilter: `!(matchesAny(attributes["path"], ["(?:^|/)@octokit/auth-token/README\\.md$"]))`,
					},
					"github-oauth": {
						RuleID:     "github-oauth",
		...
		},
		{
			...
			cfg: Config{
				...
				Rules: map[string]Rule{"aws-access-key": {
					...
					Prefilter: `!(attributes["git.sha"] in ["allowthiscommit"])`,
				}},
			},
		},
		{
			...
			cfg: Config{
				...
				Rules: map[string]Rule{"aws-access-key": {
					...
					Prefilter: `!(matchesAny(attributes["path"], [".go"]))`,
				}},
			},
		},
		{
			...
			cfg: Config{
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Prefilter: `!(matchesAny(attributes["path"], ["something.py"]))`,
					},
				},
			},
		},
			...
			cfg: Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Prefilter: `!((matchesAny(attributes["path"], ["ignore\\.xaml"]) || attributes["git.sha"] in ["abcdefg1"]))`,
						Filter:    `!(containsAny(finding["secret"], ["fake"])) && !((matchesAny(finding["line"], ["foo.+bar"]) || containsAny(finding["secret"], ["example"])))`,
					},
				},
			},
		},
			...
			cfg: Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Prefilter: `!(matchesAny(attributes["path"], ["ignore\\.xaml"]))`,
						Filter:    `!(containsAny(finding["secret"], ["fake"])) && !((matchesAny(attributes["path"], ["ignore\\.xaml"]) && attributes["git.sha"] in ["abcdefg1"] && matchesAny(finding["line"], ["foo.+bar"]) && containsAny(finding["secret"], ["example"])))`,
					},
				},
			},
		},
