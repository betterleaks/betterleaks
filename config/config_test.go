			...
			cfg: Config{
				Rules: map[string]Rule{
					"github-app-token": {
						...
						Filter: `((matchesAny(attributes["path"], [r"""(?:^|/)@octokit/auth-token/README\.md$"""]) || ("fs.windows_path" in attributes && matchesAny(attributes["fs.windows_path"], [r"""(?:^|/)@octokit/auth-token/README\.md$"""]))))`,
					},
					"github-oauth": {
						RuleID:     "github-oauth",
		...
		},
		{
			...
			cfg: Config{
				...
				Rules: map[string]Rule{"aws-access-key": {
					...
					Filter: `((matchesAny(attributes["path"], [".go"]) || ("fs.windows_path" in attributes && matchesAny(attributes["fs.windows_path"], [".go"]))))`,
				}},
			},
		},
		{
			...
			cfg: Config{
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Filter: `((matchesAny(attributes["path"], ["something.py"]) || ("fs.windows_path" in attributes && matchesAny(attributes["fs.windows_path"], ["something.py"]))))`,
					},
				},
			},
		},
			...
			cfg: Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Filter: `(((matchesAny(attributes["path"], [r"""ignore\.xaml"""]) || ("fs.windows_path" in attributes && matchesAny(attributes["fs.windows_path"], [r"""ignore\.xaml"""]))) || attributes["git.sha"] in ["abcdefg1"]))` + "\n" + `|| (containsAny(finding["secret"], ["fake"]))` + "\n" + `|| ((matchesAny(finding["line"], ["foo.+bar"]) || containsAny(finding["secret"], ["example"])))`,
					},
				},
			},
		},
			...
			cfg: Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Filter: `(containsAny(finding["secret"], ["fake"]))` + "\n" + `|| (((matchesAny(attributes["path"], [r"""ignore\.xaml"""]) || ("fs.windows_path" in attributes && matchesAny(attributes["fs.windows_path"], [r"""ignore\.xaml"""]))) && attributes["git.sha"] in ["abcdefg1"] && matchesAny(finding["line"], ["foo.+bar"]) && containsAny(finding["secret"], ["example"])))`,
					},
				},
			},
		},
