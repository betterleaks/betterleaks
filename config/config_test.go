			...
			cfg: &Config{
				Rules: map[string]Rule{
					"github-app-token": {
						...
						Tags:     []string{},
						Keywords: []string{},
						Filter:   `matchesAny(attributes[?"path"].orValue(""), ["(?:^|/)@octokit/auth-token/README\\.md$"])`,
					},
					"github-oauth": {
						RuleID:   "github-oauth",
		...
		},
		{
			...
			cfg: &Config{
				...
				Rules: map[string]Rule{"aws-access-key": {
					...
					Keywords:    []string{},
					Tags:        []string{"key", "AWS"},
					Filter:      `matchesAny(attributes[?"path"].orValue(""), [".go"])`,
				}},
			},
		},
		{
			...
			cfg: &Config{
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Keywords:    []string{},
						Tags:        []string{"key", "AWS"},
						Filter:      `matchesAny(attributes[?"path"].orValue(""), ["something.py"])`,
					},
				},
			},
		},
			...
			cfg: &Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Keywords:    []string{},
						Tags:        []string{"key", "AWS"},
						Filter:      `containsAny(finding["secret"], ["fake"])` + "\n|| " + `(matchesAny(attributes[?"path"].orValue(""), ["ignore\\.xaml"]) && attributes[?"git.sha"].orValue("") in ["abcdefg1"] && matchesAny(finding["line"], ["foo.+bar"]) && containsAny(finding["secret"], ["example"]))`,
					},
				},
			},
		},
