			...
			cfg: Config{
				Rules: map[string]Rule{
					"github-app-token": {
						...
						Prefilter: `(matchesAny(attributes["path"], [r"""(?:^|/)@octokit/auth-token/README\.md$"""]))`,
					},
					"github-oauth": {
						RuleID:     "github-oauth",
			...
			cfg: Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Prefilter: `((matchesAny(attributes["path"], [r"""ignore\.xaml"""]) || attributes["git.sha"] in ["abcdefg1"]))`,
			...
			cfg: Config{
				...
				Rules: map[string]Rule{
					"aws-secret-key-again-again": {
						...
						Prefilter: `(matchesAny(attributes["path"], [r"""ignore\.xaml"""]))`,
						Filter: `(containsAny(finding["secret"], ["fake"]))` + "\n" + `|| ((matchesAny(attributes["path"], [r"""ignore\.xaml"""]) && attributes["git.sha"] in ["abcdefg1"] && matchesAny(finding["line"], ["foo.+bar"]) && containsAny(finding["secret"], ["example"])))`,
					},
				},
			},
		},
