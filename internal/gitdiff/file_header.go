...
// ParseFileHeader synchronously parses one Git file header, starting at its
// "diff --git" line. It parses only file metadata, leaving commit metadata and
// hunk content to the caller. It returns nil if the input is not a Git header.
func ParseFileHeader(header string) (*File, error) {
	...
	return p.ParseGitFileHeader()
}

// headerReader lends lines from an existing string without a buffered reader
// or a copy of each line. The returned file may retain parts of this string.
type headerReader string

func (r *headerReader) ReadString(delim byte) (string, error) {
	text := string(*r)
	if i := strings.IndexByte(text, delim); i >= 0 {
		*r = headerReader(text[i+1:])
		return text[:i+1], nil
	}
	*r = ""
	return text, io.EOF
